...
import com.kamalkavin96.VibeXBackend.repository.SongArtistRepository;
...
public class SongServiceImpl implements SongService {
        ...
        public SongResponse createWithFile(
                ...
                Album album = albumRepository.findById(
                                request.getAlbumId()).orElseThrow(() -> new RuntimeException("Album not found"));

                String songKey = minioStorageService.uploadFile(
                                file,
                                "song-files",
                                "songs");

                String thumbnailKey = minioStorageService.uploadFile(
                                thumbnailFile,
                                "song-files",
                                "thumbnails");
        ...
        public SongResponse update(
                ...
                Song song = songRepository.findById(
                                request.getId()).orElseThrow(() -> new RuntimeException("Song not found"));
                ...
                songArtistRepository.deleteBySong(song);

                for (UUID artistId : request.getArtistIds()) {

                        Artist artist = artistRepository.findById(
                                        artistId).orElseThrow(() -> new RuntimeException("Artist not found"));
        ...
        private SongResponse buildResponse(
                        Song song) {

                List<ArtistResponse> artists = songArtistRepository
                                .findBySong(song)
                                .stream()
                                .map(mapping -> ArtistResponse.builder()
                                                .id(mapping.getArtist().getId())
                                                .name(mapping.getArtist().getName())
                                                .stageName(mapping.getArtist().getStageName())
                                                .profileImageUrl(mapping.getArtist().getProfileImageUrl())
                                                .build())
                                .toList();
                ...
                return SongResponse.builder()
                                ...
                                .songKey(song.getSongKey())
                                .thumbnailKey(song.getThumbnailKey())
